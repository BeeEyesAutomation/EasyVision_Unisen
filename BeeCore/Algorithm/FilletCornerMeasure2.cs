...
namespace BeeCore.Algorithm
{
    ...
    public class FilletCornerMeasure2
    {
        ...
        public double MinLineSpanPx = 20.0;     // minimum span (projection length) to accept as straight

        // ---- Helpers for local straightness ----
        private (double lineErr, double circleErr, double span, double dThetaDeg)
        ComputeLocalStraightness(Point[] contour, int idx, int win)
        {
            int n = contour.Length;
            if (n == 0) return (double.MaxValue, double.MaxValue, 0, 180);
            win = Math.Max(1, Math.Min(win, n/2));
            var pts = new List<Point2f>(2*win+1);
            int i0 = (idx - win + n) % n;
            for (int k = 0; k <= 2*win; k++)
            {
                var p = contour[(i0 + k) % n];
                pts.Add(new Point2f(p.X, p.Y));
            }

            // Fit line using OpenCV (returns direction + a point)
            var f = Cv2.FitLine(pts.ToArray(), DistanceTypes.L2, 0, 0.01, 0.01);
            float vx = (float)f.Vx, vy = (float)f.Vy, x0 = (float)f.X1, y0 = (float)f.Y1;
            float a = vy, b = -vx, c = -(a * x0 + b * y0); // normalized later
            double ab = Math.Sqrt(a*a + b*b); if (ab < 1e-6) ab = 1;
            a = (float)(a/ab); b = (float)(b/ab); c = (float)(c/ab);

            // line error & span along direction (vx,vy)
            double sum = 0, maxProj = double.NegativeInfinity, minProj = double.PositiveInfinity;
            foreach (var p in pts)
            {
                double d = Math.Abs(a*p.X + b*p.Y + c);
                sum += d;
                double proj = (p.X - x0)*vx + (p.Y - y0)*vy;
                if (proj > maxProj) maxProj = proj;
                if (proj < minProj) minProj = proj;
            }
            double lineErr = sum / pts.Count;
            double span = Math.Abs(maxProj - minProj);

            // Simple algebraic circle fit (Kåsa)
            ...
            double D=sol[0], E=sol[1];
            double cx = -D/2.0, cy = -E/2.0;

            // proxy error: mean absolute deviation from radial alignment (no sqrt)
            double circleSum = 0;
            foreach (var p in pts)
                circleSum += Math.Abs((p.X-cx)*(p.X-cx) + (p.Y-cy)*(p.Y-cy));
            double circleErr = circleSum / pts.Count;

            // local turning angle at idx
            var prev = contour[(idx-1+n)%n]; var curr = contour[idx]; var next = contour[(idx+1)%n];
            double ang1 = Math.Atan2(curr.Y - prev.Y, curr.X - prev.X);
            double ang2 = Math.Atan2(next.Y - curr.Y, next.X - curr.X);
            double dth = Math.Abs((ang2-ang1) * 180.0/Math.PI); if (dth>180) dth = 360-dth;

            return (lineErr, circleErr, span, dth);
        }
        ...
        private static double[] Solve3x3(double[,] A, double[] b)
        {
            ...
            if (Math.Abs(det)<1e-9) return new[]{0.0,0.0,0.0};
            double invDet = 1.0/det;
            double c00 =  (a11*a22 - a12*a21)*invDet;
            double c01 = -(a01*a22 - a02*a21)*invDet;
            double c02 =  (a01*a12 - a02*a11)*invDet;
            double c10 = -(a10*a22 - a12*a20)*invDet;
            double c11 =  (a00*a22 - a02*a20)*invDet;
            double c12 = -(a00*a12 - a02*a10)*invDet;
            double c20 =  (a10*a21 - a11*a20)*invDet;
            double c21 = -(a00*a21 - a01*a20)*invDet;
            double c22 =  (a00*a11 - a01*a10)*invDet;
            return new[]{
                c00*b[0]+c01*b[1]+c02*b[2],
                c10*b[0]+c11*b[1]+c12*b[2],
                c20*b[0]+c21*b[1]+c22*b[2]
            };
        }

        private bool IsLocallyStraight(Point[] contour, int idx)
        {
            var (lineErr, circleErr, span, dth) = ComputeLocalStraightness(contour, idx, LocalWindow);
            if (span < MinLineSpanPx) return false;
            if (lineErr * StraightGain >= circleErr) return false;
            if (dth > CurvThreshDeg) return false;
            return true;
        }
public Line2D ToLine2D(LineAB L)
        ...
        public Result Measure(Mat image, Mat edges, bool debugDraw = false, AxisOption axis = AxisOption.Both, bool signedDistance = false)
        {
            ...
            if (PairStrategy == LinePairStrategy.StrongPlusContourOrth)
            {
                ...
                // >>> Ưu tiên các điểm "thẳng cục bộ" để tránh bám vào cung tròn
                var straightIdx = new List<int>();
                for (int ii = 0; ii < contour.Length; ++ii)
                    if (IsLocallyStraight(contour, ii)) straightIdx.Add(ii);

                IEnumerable<Point2f> pool;
                if (straightIdx.Count > 0)
                    pool = straightIdx.Select(i => new Point2f(contour[i].X, contour[i].Y));
                else
                    pool = candPts; // fallback

                // (3) chọn điểm xa nhất THEO PHƯƠNG CỦA t1 (max |projection|) trong pool
        ...
        private FilletCornerMeasure2.LineAB EnsureNormalInwardsFromO(
            FilletCornerMeasure2.LineAB L, Point[] contour, Point2f O)
        {
        ...
        private FilletCornerMeasure2.LineAB BuildPerpLineThroughPoint(
            FilletCornerMeasure2.LineAB best, Point2f passPoint)
        {
            ...
            return new FilletCornerMeasure2.LineAB { A = A2, B = B2, C = C2, P1 = P0, P2 = P1 };
        }
        ...
        private Point2f NearestIntersectionToPoint(LineAB L, Point[] contour, Point2f P0, out bool ok)
        {
            ...
            // Ưu tiên các giao điểm nằm trên đoạn thẳng cục bộ
            bool pickedStraight = false;
            foreach (var q in ints)
            {
                int idx = ContourGeometry.NearestIndex(contour, q);
                if (idx >= 0 && IsLocallyStraight(contour, idx))
                {
                    double dx = q.X - P0.X, dy = q.Y - P0.Y;
                    double d2 = dx * dx + dy * dy;
                    if (d2 < best) { best = d2; bestX = q; pickedStraight = true; }
                }
            }
        ...
        private Point2f ClosestPointToLineNear(Point[] contour, LineAB L, Point2f P0, double eps = 0)
        {
            ...
            // Ưu tiên điểm gần line và thuộc đoạn thẳng cục bộ
            bool pickedStraight = false;
            for (int i = 0; i < contour.Length; ++i)
            {
                ...
                if (dLine <= eps)
                {
                    if (IsLocallyStraight(contour, i))
                    {
                        double dO2 = (p.X - P0.X) * (p.X - P0.X) + (p.Y - P0.Y) * (p.Y - P0.Y);
                        if (dO2 < best) { best = dO2; bestP = new Point2f(p.X, p.Y); pickedStraight = true; }
                    }
                }
            }
            if (!pickedStraight)
