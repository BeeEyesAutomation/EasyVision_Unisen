    ...
    public class Edge2
    {
        ...
        public void DoWork(RectRotate rotArea, RectRotate rotMask)
        {
            ...
            using (Mat raw = BeeCore.Common.listCamera[IndexCCD].matRaw.Clone())
            {
                ...
                // Filter component theo length giúp loại fragment ngắn ở rìa (noise vành ngoài)
                // mà vẫn giữ được line dài thực sự — quan trọng khi ROI lớn.
                if (IsClearNoiseBig)
                ...
                // (vd. Edge2 inherit rotation từ Position_Adjustment).
        ...
        public void Complete()
        {
            ...
            //        break;
            //}
            if (ParallelLines.Found == true)
                WidthResult = ParallelLines.GapMm;
            else if(Line2DCli.Found == true)
            ...
            PerValue = Line2DCli.Inliers > 0 ? (float)((Line2DCli.LengthPx / Line2DCli.Inliers) * 100.0) : 0;
            //if (PerValue > 70)
            //{
            //    Common.TryGetTool(IndexThread, Index).Results = Results.OK;
            //}
        ...
        public Graphics DrawResult(Graphics gc)
        {
            ...
            gc.DrawString($"{WidthResult:F2}mm L={Line2DCli.LengthMm:F2} +"+PerValue+"%", new Font("Arial", Global.ParaShow.FontSize), new SolidBrush(Global.ParaShow.ColorInfor), p1.X + 5, (p1.Y + p2.Y) / 2 + 10);
            // gc.ResetTransform();
            //mat = new Matrix();
            //if (!Global.IsRun)
            //{
