...
public class AllenBradleyPcccNet : NetworkConnectedCip
{
	...
	public override OperateResult<string> ReadString(string address, ushort length, Encoding encoding)
	{
		if (!string.IsNullOrEmpty(address) && address.StartsWith("ST"))
		{
			...
			OperateResult<byte[]> operateResult2 = Read(address, (ushort)((length % 2 != 0) ? (length + 3) : (length + 2)));
			if (!operateResult2.IsSuccess)
			{
	...
	public override async Task<OperateResult<string>> ReadStringAsync(string address, ushort length, Encoding encoding)
	{
		if (!string.IsNullOrEmpty(address) && address.StartsWith("ST"))
		{
			...
			OperateResult<byte[]> read3 = await ReadAsync(address, (ushort)((length % 2 != 0) ? (length + 3) : (length + 2)));
			if (!read3.IsSuccess)
			{
				return OperateResult.CreateFailedResult<string>(read3);
			}
			int len3 = base.ByteTransform.TransUInt16(read3.Content, 0);
			if (len3 + 2 > read3.Content.Length)
			{
				len3 = read3.Content.Length - 2;
			}
			return OperateResult.CreateSuccessResult(encoding.GetString(SoftBasic.BytesReverseByWord(read3.Content), 2, len3));
		}
		return base.ReadString(address, length, encoding);
	}
