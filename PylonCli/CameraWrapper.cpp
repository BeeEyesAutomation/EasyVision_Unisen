...
// PylonCli owns the Basler/Pylon SDK boundary. Managed callers should use
// BeeCore.Camera and preserve SDK grab lifecycle sequencing.
std::atomic<int> _bufIndex{ 0 }; // 0=A, 1=B (buffer dã publish)
...
bool ApplyBandwidth(Pylon::CInstantCamera* cam, int mbPerSec)
{
    try {
        ...
        // Packet size (gi?ng Pylon Viewer)
...
void Camera::Open(System::String^ name) {
    try {
       ...
       // auto& nm = _cam->GetNodeMap();
        ...
        // 1?? Set transport l?i (gi?ng reset)
       /* if (auto p = GenApi::CIntegerPtr(nm.GetNode("GevSCPSPacketSize")))
        ...
        //// 2?? Warm-up grab vài frame
        //_cam->StartGrabbing(5);
        //// ch? grab xong (timeout d? kh?i treo)
        //int wait = 0;
...
bool Camera::SetFps(float microseconds)
{
    try {
        ...
        if (CFloatPtr fps = nm.GetNode("AcquisitionFrameRateAbs"))
            if (IsWritable(fps))
            {
                fps->SetValue(microseconds); // ví d? 10 fps
                _lastError = "";
                // 2?? Packet size = max
                if (auto p = GenApi::CIntegerPtr(nm.GetNode("GevSCPSPacketSize")))
                    if (IsWritable(p)) p->SetValue(p->GetMax());

                // 3?? Inter-packet delay = 0
                if (auto p = GenApi::CIntegerPtr(nm.GetNode("GevSCPD")))
...
float Camera::SetWidth(float v)
{
    ...
    try {
        ...
        // D?ng r?i khôi ph?c dúng tr?ng thái ban d?u
        bool wasGrabbing = _cam->IsGrabbing();
        if (wasGrabbing)_cam->StopGrabbing();
        if (!node) { _lastError = "Node Width không t?n t?i!"; return 0.0f; }
...
float Camera::SetHeight(float v)
{
    ...
    try {
        ...
        bool wasGrabbing = _cam->IsGrabbing();
        if (wasGrabbing)_cam->StopGrabbing();
        if (!node) { _lastError = "Node Height không t?n t?i!"; return 0.0f; }
...
float Camera::SetOffsetX(float v)
{
    ...
    try {
        ...
        // M?t s? model yêu c?u CenterX = false d? set OffsetX
...
float Camera::SetExposure(float us)
{
    ...
    try {
        ...
        if (f && GenApi::IsWritable(f)) {
            ...
            // clamp + (n?u có inc>0) làm tròn theo bu?c
            double val = us;
            if (val < mn) val = mn;
            if (val > mx) val = mx;
            if (inc > 0.0) {
        ...
        // 2) Integer: ExposureTimeRaw (+ factor n?u có)
...
static double RoundToIncClamp_D(double desired, double vmin, double vmax, double inc)
{
    ...
    if (desired > vmax) desired = vmax;
    if (inc > 0.0) {
        const double n = std::floor((desired - vmin) / inc + 0.5); // làm tròn v? b?i s? inc
        desired = vmin + n * inc;
        if (desired < vmin) desired = vmin;
        if (desired > vmax) desired = vmax;
    }
    ...
    try {
        ...
        // ===== 1) Float: Gain (dB) =====
        {
            ...
            if (f && GenApi::IsWritable(f)) {
                ...
                f->SetValue(val);
                return static_cast<float>(f->GetValue()); // tr? v? dB th?c t?
            }
        }
        ...
        // ===== 3) Integer: GainRaw (ticks) =====
        {
            ...
            if (i && GenApi::IsWritable(i)) {
                ...
                // N?u có h? s? d?i raw->dB (tùy hãng), có th? tr? v? dB; m?c d?nh tr? v? raw
                // Ví d? (tùy model): CFloatPtr fac = nm.GetNode("GainRawToDbFactor");
                // if (fac && GenApi::IsReadable(fac)) return static_cast<float>(i->GetValue() * fac->GetValue());
                return static_cast<float>(i->GetValue());
            }
        }

        // ===== 4) Bi?n th? Basler: BslAnalogGain / BslDigitalGain (float) =====
        {
        ...
        // ===== 5) Bi?n th? Basler: GainRaw tuong t? =====
        {
...
static int64_t RoundToStepClamp(int64_t desired, int64_t vmin, int64_t vmax, int64_t inc)
{
    ...
    if (desired < vmin) desired = vmin;
    if (desired > vmax) desired = vmax;
    // làm tròn v? b?i s? g?n nh?t c?a inc tính t? vmin
    ...
    try {
        ...
        // 1) Integer: DigitalShift
        {
            ...
            if (ishift && GenApi::IsWritable(ishift)) {
                ...
                // d?c l?i d? bi?t ch?c giá tr? th?c t?
                int64_t applied = ishift->GetValue();
                return static_cast<float>(applied);
            }
        }

        // 2) Integer: BslDigitalShift (bi?n th? Basler)
        {
        ...
        // 3) Enumeration: DigitalShift (ít g?p)
        {
            CEnumerationPtr eshift = nm.GetNode("DigitalShift");
            if (eshift && GenApi::IsWritable(eshift)) {
                int64_t desired = static_cast<int64_t>(llround(value));

                // th? set tr?c ti?p theo int (nhi?u model map tr?c ti?p)
                try {
                    ...
                    // fallback: tìm entry kh? d?ng g?n nh?t
...
void Camera::GetDigitalShift(float% min, float% max, float% step, float% current)
{
    ...
    try {
        ...
        if (ishift && GenApi::IsReadable(ishift)) {
            ...
            step = static_cast<float>(vinc);   // integer ? bu?c r?i r?c (thu?ng = 1)
            current = static_cast<float>(vcur);
            return;
        }

        // --- 2) Integer: BslDigitalShift (d?t riêng c?a Basler) ---
        CIntegerPtr bsl = nm.GetNode("BslDigitalShift");
        ...
        // --- 3) Enumeration: DigitalShift (ít g?p, d? phòng) ---
        CEnumerationPtr eshift = nm.GetNode("DigitalShift");
...
void Camera::GetGain(float% min, float% max, float% step, float% current)
{
    ...
    try {
        ...
        // (tu? ch?n) t?t auto d? d?c/ghi ?n d?nh
        ...
        if (sel && GenApi::IsWritable(sel)) {
            // Cách 1: th? b?ng tên "All"
            CEnumEntryPtr all = sel->GetEntryByName("All");
            if (all && GenApi::IsAvailable(all)) {
                sel->SetIntValue(all->GetValue());
            }
            else {
                // Cách 2: duy?t entries, ch?n entry kh? d?ng d?u tiên
                GenApi::NodeList_t entries;
        ...
        // 1) Uu tiên node float chu?n: Gain (thu?ng là dB)
        CFloatPtr f = nm.GetNode("Gain");
        if (f && IsReadable(f)) {
            min = static_cast<float>(f->GetMin());
            max = static_cast<float>(f->GetMax());
        ...
        // 2) Fallback legacy: GainAbs (m?t s? thi?t b? dùng tên này)
        ...
        // 3) Fallback integer: GainRaw (don v? "ticks")
        CIntegerPtr i = nm.GetNode("GainRaw");
        if (i && IsReadable(i)) {
            ...
            if (incRaw <= 0) incRaw = 1;      // integer nên d?m b?o >= 1
        ...
        // 4) (tu? hãng) AnalogGain / DigitalGain
...
void Camera::GetBlackLevel(float% min, float% max, float% step, float% current)
{
    ...
    try {
        ...
        // 1) (tu? ch?n) ch?n selector = All n?u có
        CEnumerationPtr sel = nm.GetNode("BlackLevelSelector");
        if (sel && GenApi::IsWritable(sel)) {
            // uu tiên entry "All" n?u có
            CEnumEntryPtr all = sel->GetEntryByName("All");
            if (all && GenApi::IsAvailable(all)) {
                try { sel->SetIntValue(all->GetValue()); }
                catch (...) {}
            }
            // n?u không có "All", d? nguyên selector hi?n t?i
        }

        // 2) Uu tiên node float chu?n: BlackLevel (don v? DN)
        {
        ...
        // 3) Fallback integer: BlackLevelRaw (d?m thô)
        {
            ...
            if (iraw && GenApi::IsReadable(iraw)) {
                ...
                min = static_cast<float>(vmin);
                max = static_cast<float>(vmax);
                step = static_cast<float>(vinc); // integer ? bu?c r?i r?c
                current = static_cast<float>(vcur);
                return;
            }
        }

        // 4) Bi?n th? Basler: BslBlackLevel (float)
        {
        ...
        // 5) Bi?n th? Basler: BslBlackLevelRaw (integer)
        {
        ...
        // outputs dã là 0; return không throw
    }
    catch (const GenICam::GenericException& e) {
...
void Camera::ProcessGrabbed(const CGrabResultPtr& ptr)
{
    ...
    // Convert vào buffer chua publish
    _conv->Convert(*target, ptr);

    // Publish buffer m?i (SAU convert)
    _bufIndex.store(next, std::memory_order_release);

    // FPS count – CH? khi có frame th?t
    _frameCount++;
...
System::IntPtr Camera::GrabOneUcharPtr(int timeoutMs, int% w, int% h, int% stride, int% channels)
{
    ...
    try {
        ...
        // Vòng l?p “ch? trong ngân sách timeout”
        while (true) {
            ...
            // dùng ph?n timeout còn l?i cho l?n g?i này
            const int remain = timeoutMs - waited;
            if (!_cam->RetrieveResult(remain, res, TimeoutHandling_Return)) {
                // l?n này chua có frame ? loop ti?p cho d?n h?t timeout
                continue;
            }

            if (!res || !res->GrabSucceeded()) {
                // Có k?t qu? nhung l?i – ghi mã l?i r?i loop ti?p cho d?n h?t timeout
                if (res) {
            ...
            // Convert theo c?u hình OutputPixelFormat/OutputPaddingX dã set
            ...
            _lastError = System::String::Empty;
            return System::IntPtr((void*)dst->GetBuffer()); // buffer n?i b?
        }
    }
...
IntPtr Camera::CopyLatestImage(int% w, int% h, int% stride, int% channels)
{
    ...
    CPylonImage* src = CurrentBuffer();   // ? buffer dã ?n d?nh
...
//{
//    w = h = stride = channels = 0;
//    CPylonImage* src = (_bufIndex == 0) ? _bufB : _bufA; // l?y ?nh v?a hoàn t?t
//    if (!src || !src->IsValid()) {
...
{
    return _emaFps; // s? >0 sau khi có vài frame
}
