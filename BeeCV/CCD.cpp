...
#include <locale>
#include <codecvt>
#include <atlstr.h> // C?n thu vi?n ATL/MFC


using namespace CvPlus; 
using namespace System::Runtime::InteropServices;

// BeeCV owns the native MVS/USB camera wrapper surface. Managed UI/core code
// should preserve Start -> Read -> Stop lifecycle ordering through BeeCore.Camera.
namespace {
	struct GrabPause {
		CMvCamera* cam;
		bool stopped = false;
	...
	// Làm tròn theo step (step <= 0 thì b? qua)
	inline double round_to_step(double v, double minV, double step) {
		if (step <= 0.0) return v;
		return std::round((v - minV) / step) * step + minV;
	}

	// Th? t?t Auto n?u tham s? liên quan d?n Exposure/Gain (không fail toàn b? n?u không h? tr?)
	inline void try_disable_auto(CMvCamera* cam, const std::string& key) {
		...
		if (contains("Exposure") || contains("ExposureTime")) {
			// 0 = Off d?i v?i enum ExposureAuto c?a HIK/MVS
	...
	bool probe_float_increment(CMvCamera* cam, const string& key, double& incOut)
	{
		...
		// Th? d?c FLOAT node ph?
		for (auto suf : SUFFIXES) {
			string nid = key + suf;
			MVCC_FLOATVALUE fv{};
			if (cam->GetFloatValue(nid.c_str(), &fv) == MV_OK) {
				// Tu? model: dùng fCurValue ho?c fMin tu? cách SDK công b?
				double inc = (fv.fCurValue > 0) ? fv.fCurValue : fv.fMin;
				if (inc > 0) { incOut = inc; return true; }
			}
		}
		// Th? d?c INT node ph? (m?t s? SDK công b? step d?ng INT)
		for (auto suf : SUFFIXES) {
			string nid = key + suf;
			MVCC_INTVALUE_EX iv{};
...
float CCD::SetPara(int indexCCD, System::String^ nameParaManaged, float valueIn)
{
	...
	// T? t?t ch? d? Auto n?u có (không coi là l?i n?u không h? tr?)
	try_disable_auto(cam, key);

	// Ð?m b?o luôn StartGrabbing l?i nh? RAII
	GrabPause guard(cam);

	// 1) Th? nhu FLOAT tru?c
	{
		MVCC_FLOATVALUE fv{}; // min/max/inc d?ng double
		if (cam->GetFloatValue(key.c_str(), &fv) == MV_OK) {
			double v = (double)valueIn;
			// clamp min–max

			v =ClampI(v, fv.fMin, fv.fMax);
			// làm tròn theo step n?u có
		//	v = round_to_step(v, fv.fMin, fv.fInc);

			if (cam->SetFloatValue(key.c_str(), v) == MV_OK) {
				// Ð?c l?i giá tr? th?c s? áp d?ng (n?u mu?n chính xác)
				MVCC_FLOATVALUE fv2{};
			...
			// N?u set float th?t b?i, d?ng v?i tr? v? — th? sang INT bên du?i
		}
	}

	// 2) Th? nhu INT (m?t s? node là integer nhu Width/Height/OffsetX/OffsetY/Exposure lines trên vài model)
	{
		MVCC_INTVALUE_EX iv{};
	...
	// 3) Không h? tr? ho?c set th?t b?i
...
extern "C" __declspec(dllexport) void SetRaw(uchar* uc, int image_rows, int image_cols, int image_type)
{
	...
	//// PH?I clone n?u uc là vùng nh? c?p t? C#
...
System::String^ ScanHik()
{
	...
	// ch:????????? | en:Enumerate all devices within subnet
	...
	// ch:???????????????? | en:Add value to the information list box and display
...
)
{
	...
	// 1) Th? FLOAT tru?c
	{
		...
		if (cam->GetFloatValue(key.c_str(), &fv) == MV_OK) {
			...
			// C? g?ng dò step; n?u không có ? d? 0 (UI tu? ch?n x? lý: free slider / nh?p t? do)
			double inc = 0.0;
			if (probe_float_increment(cam, key, inc)) {
				stepVal = static_cast<float>(inc);
			}
			else {
				stepVal = 0.0f; // không công b? step cho FLOAT
			}
			IsWaiting = false;
			//typeName = "Float";
			return true;
	...
	// 2) N?u không ph?i FLOAT, th? INT
	{
		...
		if (cam->GetIntValue(key.c_str(), &iv) == MV_OK) {
			...
			// nInc có th? =0 ? vài node; chu?n hoá v? >=1
	...
	IsWaiting = false;
	// 3) Không d?c du?c
	std::cerr << "? Không d?c du?c thông tin tham s?: " << key << std::endl;
	return false;
}
...
bool ConnectUsb( int index)
{
	...
	camUSB.set(cv::CAP_PROP_FPS, 60);          // Tang s? khung hình trên giây
...
static bool DeviceReset(CMvCamera* camera, int waitMsAfter = 3000)
{
	...
	camera->StopGrabbing(); // an toàn: d?ng stream n?u dang ch?y

	int r = camera->CommandExecute("DeviceReset"); // g?i GenICam command
	if (r != MV_OK) {
		std::cerr << "DeviceReset unsupported/failed: " << r << "\n";
		return false; // ho?c fallback: close/reopen
	}
...
bool ConnectHik( int index,int indexCCD)
{
	...
	// ch:??????????? | en:Device instance created by device information
...
//}
//
//// (Tu? ch?n) zero-copy cho BGR8: ch? dùng trong scope hi?n t?i
//struct BorrowedFrame {
//	cv::Mat view; // KHÔNG s? h?u b? nh?
//	struct Guard {
...
// Chính: l?y frame ra Mat BGR (an toàn, s? h?u b? nh?)
...
//	size_t step = 0;
//#ifdef MV_FRAME_OUT_INFO_EX_HAS_STRIDE
//	step = info.nStride; // n?u SDK có nStride thì dùng
//#endif
//
//	switch (px)
//	{
...
//		cv::cvtColor(raw, imageBGR, cv::COLOR_BayerBG2BGR); // chu?n BGR
//		break;
//	}
...
static inline bool ConvertBySDKEx(CMvCamera* camera, const MV_FRAME_OUT& out, cv::Mat& dstBGR) {
	...
	prm.nHeight = h;
	prm.enSrcPixelType = (MvGvspPixelType)out.stFrameInfo.enPixelType;
	prm.pSrcData = (unsigned char*)out.pBufAddr;      // cast dúng ki?u
	prm.nSrcDataLen = out.stFrameInfo.nFrameLen;         // dùng frameLen t? SDK
...
//	if (!p || w == 0 || h == 0) return false;
//
//	// --- 1) BGR8: copy th?ng (1 l?n) vào buffer tái s? d?ng
...
//	}
//
//	// --- 2) RGB8: d?o kênh ? BGR (chu?n OpenCV)
...
//		return true;
//	}
//	// --- 4) Bayer8: demosaic b?ng OpenCV (nhanh). Có 4 pattern thu?ng g?p.
//	// N?u b?n mu?n màu d?p hon (HQ), có th? b? 4 nhánh này d? dùng SDK ConvertBySDKEx.
//	/*if (px == PixelType_Gvsp_BayerBG8) {
//		EnsureSize(imageBGR, (int)w, (int)h, CV_8UC3);
//		cv::Mat raw(h, w, CV_8UC1, p);
...
//	}*/
//
//	// --- 5) Các d?nh d?ng còn l?i (Bayer10/12/16, Mono10/12/16, Packed, v.v.) ? dùng SDK EX
//	return ConvertBySDKEx(camera, out, imageBGR);
//}
bool CaptureFrame(CMvCamera* camera, cv::Mat& imageBGR) {
	MV_FRAME_OUT out = { 0 };

	// L?y buffer ?nh t? camera (timeout 1000 ms)
	...
	// RAII: luôn tr? buffer v? SDK khi ra kh?i scope
	struct BufferGuard {
		CMvCamera* cam; MV_FRAME_OUT* o;
	...
	// T?o Mat "view" tr? tr?c ti?p vào buffer c?a SDK (không copy)
	auto view8UC1 = [&](void* buf) { return cv::Mat(h, w, CV_8UC1, buf); };
	auto view8UC3 = [&](void* buf) { return cv::Mat(h, w, CV_8UC3, buf); };

	switch (pix) {
	case PixelType_Gvsp_BGR8_Packed: {
		// Ðã là BGR8: copy ra imageBGR d? dùng ngoài ph?m vi buffer SDK
	...
	case PixelType_Gvsp_Mono12: {
		// N?u SDK có hàm convert, uu tiên dùng (ví d? MV_CC_ConvertPixelType…)
		// ? dây minh ho? nén 12-bit v? 8-bit b?ng d?ch ph?i 4 bit.
		// LUU Ý: N?u là "Mono12Packed", b? trí bit khác -> c?n gi?i pack dúng theo SDK!
		const int bytes = w * h * 2; // gi? d?nh m?i pixel 16-bit ch?a 12-bit h?u d?ng
...
//		// grab/retrieve d? gi?m latency
//		if (!camUSB.grab() || !camUSB.retrieve(rawBGR)) {
//			*rows = *cols = *Type = 0;
//			return nullptr;
//		}
//		// Ð?m b?o ?nh v? BGR 8-bit n?u camera tr? YUYV/GRAY…
...
//	}
//
//	// Xu?t d? li?u d?ng bytes (copy 1 l?n d? caller s? h?u b? nh?)
...
uchar* CCD::ReadCCD(int indexCCD, int* rows, int* cols, int* Type)
{
	...
	*rows = *cols = *Type = 0;
	...
	case 1: { // Camera SDK
		...
		// =====================================================
		// 2?? N?u KHÔNG ph?i Mono8 ? convert sang BGR
		// =====================================================
		else
		{
			cv::Mat dstBGR;
...
bool CCD::ReconnectHik(int indexCCD,System::String^ NameCamera, int waitAfterResetMs )
{
	...
	// ===== 1) Stop grabbing n?u còn s?ng =====
	m_pcMyCamera[indexCCD]->StopGrabbing();

	// ===== 2) Th? DeviceReset (soft reset) =====
