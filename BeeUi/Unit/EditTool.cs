...
namespace BeeUi
{
    ...
    public partial class EditTool : UserControl
    {
      ...
      public void UnResgisTer()
                            ...
                            if (Global.Config.IsMultiProg)
                            {
                                ...
                                if (Global.Config.NumTrig >1 && Global.NumProgFromPLC == Global.Config.NumTrig)
                                {
                                  ...
                                  //  opt.Direction = MergeDirection.Vertical;
                                    ...
                                    // FIX: bỏ allocation thừa - matRS sẽ được gán trong switch; nếu không match case nào,
                                    // matRegStep1=null sẽ được handle bởi null-check ở dưới (line 313)
                                    Mat matRS = null;
                        switch(Global.NumProgFromPLC)
                            ...
                            if (matRegStep1 != null)
                                    if (BeeCore.Common.listCamera[Global.IndexCCCD] != null)
                                        ...
                                        {
                                            // FIX: dispose Mat cũ trước khi gán để tránh leak; bỏ "new Mat()" thừa ngay trước Clone
                                            ...
                                            G.IsCalib = false;
                                            // FIX: dispose ảnh PictureBox cũ trước khi gán mới
        ...
        {
            // FIX C9: async void cleanup — wrap try-catch để exception khi shutdown không crash app
            try
            {
      ...
      public void LockSpilter(bool IsLock)
        ...
        {
            // FIX C9: async void event handler — wrap try-catch
            try
            {
        ...
        private void openFolderImage_Click(object sender, EventArgs e)
        {
            ...
            if (!Global.IsRun)
            {
                ...
                BeeCore.Common.listCamera[Global.IndexCCCD].matRaw = View.listMat[View.indexFile];// Cv2.ImRead(Files[indexFile]) — FIX: bỏ double-assign thừa
