...
namespace BeeUi
{
    public class FingerPrint
    {
        ...
        static string Query(string wmiClass, string prop, string where = null)
        {
            string q = $"SELECT {prop} FROM {wmiClass}" +
        ...
        // Trả về fingerprint đã băm SHA‑256 (64 ký tự hex)
        public static string Get()
        {
            // Ưu tiên UUID, rồi serial board, BIOS, kèm MachineGuid
            var pieces = new[]
            {
            Query("Win32_ComputerSystemProduct", "UUID"),
            ...
            // Ghép theo thứ tự, bỏ phần rỗng
            string raw = string.Join("|", pieces.Where(p => !string.IsNullOrEmpty(p)));
